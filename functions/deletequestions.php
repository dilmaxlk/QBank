...
$rowCount = count($_POST["checkbox"]);
            ...
            $stmt = $db->prepare("DELETE FROM cp_question WHERE que_id='" . $_POST["checkbox"][$i] . "'");
            $stmt->execute(); 
            $stmt->close();
...
$SubjectID2 = $_GET['SubjectID'];
$UnitID2 = $_GET['UnitID'];


//Jump to the same page after deleteing the image
header('Location: ../index.php?page=ViewQuestion&SubjectID='.$SubjectID2.'&UnitID='.$UnitID2); 
